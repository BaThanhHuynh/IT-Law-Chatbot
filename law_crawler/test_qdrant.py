...
print(f"🔄 Đang load model embedding từ {MODEL_PATH}...")
...
for idx, query in enumerate(QUERIES, 1):
    ...
    for i, scored_point in enumerate(results.points, 1):
        ...
        if not full_text:
            full_text = payload.get("text", "") # Fallback nếu dữ liệu không có full_dieu_text
        short_chunk = payload.get("noi_dung_chunk", payload.get("text", ""))[:180]
